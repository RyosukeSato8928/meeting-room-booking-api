...
# Read
# response_model=List[schemas.User]は、APIのレスポンスがUserモデルのリストであることを示すための引数（レスポンスの形）
...
# Create
# データベース処理後（レスポンス）のスキーマはschemas.Userとなる
@app.post("/users", response_model=schemas.User)
# 入力値：user（リクエスト）のスキーマはschemas.UserCreateとなる
async def create_users(user: schemas.UserCreate, db: Session = Depends(get_db)):
    return crud.create_user(db=db, user=user)

@app.post("/rooms", response_model=schemas.Room)
# room: schemas.Roomは、リクエストボディからユーザーデータを受け取るための引数（登録するデータ）
